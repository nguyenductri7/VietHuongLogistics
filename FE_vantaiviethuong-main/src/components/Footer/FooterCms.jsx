...
export default function FooterCms() {
  ...
  const localizedFooter = useMemo(
    () => ({ ...DEFAULT_FOOTER, ...localizeObject(footer, language) }),
    [footer, language],
  )
  ...
  const offices = useMemo(
    () => {
      const source = Array.isArray(footer.offices) && footer.offices.length ? footer.offices : DEFAULT_FOOTER.offices
      return source.map(item => ({
        ...item,
        city: getLocalizedValue(item.city, language),
        addr: getLocalizedValue(item.addr, language),
      }))
    },
    [footer.offices, language],
  )
    ...
    <footer className={styles.footer} role="contentinfo">
      <div className={styles.top}>
        <div className={styles.brand}>
          ...
          <p className={styles.companyName}>{localizedFooter.company_name}</p>
          <p className={styles.taxCode}>Mã số thuế: {localizedFooter.tax_code}</p>

          <div className={styles.socials}>
            <a href={localizedFooter.facebook_url || '#'} aria-label="Facebook" target="_blank" rel="noopener noreferrer" className={styles.socials__facebook}>
              <svg viewBox="0 0 24 24" fill="currentColor">
                <path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z" />
              </svg>
            </a>

            <a href={localizedFooter.youtube_url || '#'} aria-label="YouTube" target="_blank" rel="noopener noreferrer" className={styles.socials__youtube}>
              <svg viewBox="0 0 24 24" fill="currentColor">
                <path d="M22.54 6.42a2.78 2.78 0 00-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46a2.78 2.78 0 00-1.95 1.96A29 29 0 001 12a29 29 0 00.46 5.58A2.78 2.78 0 003.41 19.6C5.12 20 12 20 12 20s6.88 0 8.59-.46a2.78 2.78 0 001.95-1.95A29 29 0 0023 12a29 29 0 00-.46-5.58zM9.75 15.02V8.98L15.5 12l-5.75 3.02z" />
              </svg>
            </a>

            <a href={localizedFooter.instagram_url || '#'} aria-label="Instagram" target="_blank" rel="noopener noreferrer" className={styles.socials__instagram}>
            ...
            <a href={localizedFooter.zalo_url || '#'} aria-label="Zalo" target="_blank" rel="noopener noreferrer" className={styles.socials__zalo}>
            ...
            <a href={`mailto:${localizedFooter.email}`} aria-label="Gmail" className={styles.socials__gmail}>
        ...
        <div className={styles.officeCol}>
          ...
          <div className={styles.addressRow}>
            ...
            <p>{localizedFooter.address}</p>
          </div>
          <p>Hotline: <a href={telHref(localizedFooter.hotline)} className={styles.hotline}>{localizedFooter.hotline}</a></p>
          <p>Email: <a href={`mailto:${localizedFooter.email}`} className={styles.emailLink}>{localizedFooter.email}</a></p>
        </div>
      ...
      <div className={styles.bottom}>
        <p>© Copyright 2026 – {localizedFooter.company_name}</p>
